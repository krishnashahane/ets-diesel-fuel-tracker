...
export default function Transactions() {
  ...
  const load = useCallback(() => {
    ...
    if (q) p.set('q', q); if (from) p.set('from', from); if (to) p.set('to', to); if (exc) p.set('exceptions', '1');
  ...
  function exportCsv() {
    ...
    if (from) p.set('from', from); if (to) p.set('to', to); if (exc) p.set('exceptions', '1');
  ...
  return (
    <div>
      <PageHeader title="Transactions" subtitle={`${fmt(total)} records`}
        action={<button className="btn-primary" onClick={exportCsv}>Export CSV</button>} />
      <div className="card mb-4 flex flex-wrap items-end gap-3 p-4">
        <div><span className="label">Search</span><input className="input" placeholder="Vehicle / driver / bill" value={q} onChange={(e) => { setPage(1); setQ(e.target.value); }} /></div>
        ...
        {loading ? <Spinner /> : (
          <table className="min-w-full">
            <thead className="border-b border-slate-100 bg-slate-50">
              <tr>{['Date', 'Bill', 'Vehicle', 'Driver', 'Site', 'Diesel', 'Amount', 'KM', 'Mileage', 'Excess', 'Status', 'Check', 'Photos', 'Loc', 'Risk', ''].map((h, i) => <th key={i} className="th">{h}</th>)}</tr>
            </thead>
            <tbody>
              {items.map((t) => {
              ...
              {!items.length && <tr><td className="td py-8 text-center text-slate-400" colSpan={16}>No transactions found.</td></tr>}
            </tbody>
          </table>
        )}
      </div>
